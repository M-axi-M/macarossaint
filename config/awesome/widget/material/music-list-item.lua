...
function mat_list_item:layout(_, width, height)
  local content_width = width - dpi(52)
  local content_x = dpi(dpi(26))
  local layout = {}
  ...
  if self._private.suffix then
    content_width = content_width - dpi(54)
    table.insert(layout, base.place_widget_at(self._private.suffix, width - dpi(400), dpi(12), width, height))
  end
  table.insert(layout, base.place_widget_at(self._private.content, content_x, 0, content_width,width - dpi(32)))
  return layout
end

function mat_list_item:fit(_, width)
  return width, width - dpi(50)
end
